...
PSISectionWriteResultRecordPtr DefaultPSIDataStream::WriteAllSectionDataToTSStream(ITSDataStreamPtr aTSStream,
		...
		ITSDataCounterPtr aDataCounter, IPSICrc32CounterPtr aCrc32Counter, bool aIsSectionLimitBy10Bit,unsigned int RefLastSectionNumPos)
{
	unsigned short NeedSectionNums;
	unsigned short TmpCuttedSectionLength;
	...
	TmpCuttedSectionLength = ResultRec->SectionLength;
	//					aPSIConfig-SectionLength = TmpSectionLength;
	aPSIHeaderConfig->PSIHeader.SetSectionLength(TmpCuttedSectionLength);
	...
	{
		unsigned char * TmpPtr=aSrcData.get();
		TmpPtr[RefLastSectionNumPos]=aPSIHeaderConfig->PSIHeader.last_section_number;
	}
	for (unsigned char i=0; i<NeedSectionNums; i++)
	{
		...
		if (i < NeedSectionNums-1) //区分最后一个Section
		{
			...
			SResult = WriteSpecialSectionDataToTSStream(aTSStream, aPSIHeaderConfig, aSrcData,
					aSrcDataLength, i * (TmpCuttedSectionLength-9), TmpCuttedSectionLength - 9, aDataCounter,
					aCrc32Counter);
		...
		else
		{
			...
			if (aSrcDataLength > TmpCuttedSectionLength - 9)
				LastSectionDataLength = aSrcDataLength - i * (TmpCuttedSectionLength - 9);
			else
			...
			SResult = WriteSpecialSectionDataToTSStream(aTSStream, aPSIHeaderConfig, aSrcData,
					aSrcDataLength, i * (TmpCuttedSectionLength-9), LastSectionDataLength-9, aDataCounter,
					aCrc32Counter);
			aTSStream->InsertNullPacket();
			aTSStream->InsertNullPacket();
		}
	...
	ResultRec->SectionLength = TmpCuttedSectionLength;
		...
		IPSICrc32CounterPtr aCrc32Counter)
{
	...
	RawBufferPtr PSIDataBuffer;
	if(aCrc32Counter)
	{
		PSIDataBuffer.reset((unsigned char *)std::malloc(aWriteLength+8+4),&free);
		std::memset(PSIDataBuffer.get(), 0, aWriteLength+8+4);
		std::memcpy(PSIDataBuffer.get(), (unsigned char *)&(aPSIHeaderConfig->PSIHeader), 8);
	...
	{
		PSIDataBuffer.reset((unsigned char *)std::malloc(aWriteLength+8),&free);
