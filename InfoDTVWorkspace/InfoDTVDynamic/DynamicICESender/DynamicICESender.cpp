...
void MEMORYCopy(std::vector<unsigned char> &aTargetVector,unsigned char * aSrcBuffer,unsigned int aBufferLength)
{
...
void PSIDynamicICESender::SendAllBuffer(RawBufferPtr aBuffer,unsigned int aBufferSize)
{
	...
	try
	{
		while(TotalCopyLength<aBufferSize)
		{
			...
			cout<<">>>TotalCopyLength:"<<TotalCopyLength<<"\tCopyLength:"<<CopyLength<<"\t aBufferSize:"<<aBufferSize<<std::endl;
			if(CopyLength!=0)
			{
