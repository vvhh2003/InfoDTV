...
#define  DTAPI_VERSION_MINOR		4
#define  DTAPI_VERSION_BUGFIX		1
#define  DTAPI_VERSION_BUILD		121
...
struct DtCmPath {
    ...
    double  m_Phase;				// Constant phase in radians
									// Used by: DTAPI_CM_PATH_CPHASE
    double  m_Speed;				// Speed in m/s
	// Constructor - gives parameter a default value
	DtCmPath() :
		m_Type(CONSTANT_DELAY),
		m_Attenuation(6.0), m_Delay(0.0), m_Phase(0.0), m_Speed(0.0)
	{}
	bool  operator == (DtCmPath& Rhs) {
		return m_Type==Rhs.m_Type && m_Attenuation==Rhs.m_Attenuation
			&& m_Delay==Rhs.m_Delay && m_Phase==Rhs.m_Phase && m_Speed==Rhs.m_Speed;
	}
};

// Channel-modelling parameters
struct DtCmPars {
	bool  m_EnableSnr;				// Enable noise injection
	double  m_Snr;					// Signal-to-noise ratio in dB
	std::vector<DtCmPath>  m_Paths;	// Parameters per path
	// Constructor - gives parameter a default value
	DtCmPars() : m_EnableSnr(false), m_Snr(6.0) {}
	// Operators
	bool  operator == (DtCmPars& Rhs) {
		if (m_EnableSnr!=Rhs.m_EnableSnr || m_Snr!=Rhs.m_Snr)
			return false;
		if (m_Paths.size() != Rhs.m_Paths.size())
			return false;
		std::vector<DtCmPath>::iterator  It=m_Paths.begin();
		std::vector<DtCmPath>::iterator  RhsIt=Rhs.m_Paths.begin();
		for (; It!=m_Paths.end(); It++, RhsIt++)
			if (!(*It == *RhsIt))
				return false;
		return true;
	}
...
struct DtDeviceDesc {	
	...
	unsigned char  m_Ip[4];		// IP address (only valid for via IP connected devices)
};
...
struct DtDvbT2PlpPars {
	...
	int  m_NumBlocks;			// Number of FEC blocks contained in one T2 frame
...
struct DtDvbT2Pars {
	...
	int  m_Miso;				// MISO enabled, 0 or 1
	...
	int  m_FefType;				// 0 ... 15
	int  m_FefLength;			// m_FefLength != 0 enables FEF
...
#define  DTAPI_CAP_DTMB			0x00000200	// DTMB modulation
#define  DTAPI_CAP_DVBS			0x00000400	// DVB-S modulation
#define  DTAPI_CAP_DVBS2		0x00000800	// DVB-S2 modulation
#define  DTAPI_CAP_DVBT			0x00001000	// DVB-T modulation
#define  DTAPI_CAP_DVBT2		0x00002000	// DVB-T2 modulation
#define  DTAPI_CAP_ISDBT		0x00004000	// ISDBT modulation
#define  DTAPI_CAP_QAM_A		0x00008000	// QAM-A modulation
#define  DTAPI_CAP_QAM_B		0x00010000	// QAM-B modulation
#define  DTAPI_CAP_QAM_C		0x00020000	// QAM-C modulation
...
//#pragma warning (default: 4251)
	...
	void  MakeConsistent(), MakeNumSegmConsistent();
	...
	// Constructor
	DtIsdbtPars() :
		m_DoMux(false), m_FilledOut(false), m_Valid(false),
...
public:
	...
	DTAPI_RESULT  GetGenlockState(bool& Enable, int& Lock);
	DTAPI_RESULT  GetVcxoState(bool& Enable, int& Lock, int& VcxoClkFreqHz);
	DTAPI_RESULT  SetVcxoState(bool Enable, int Value);
...
public:
	...
	DTAPI_RESULT  SetModControl(int, int, int, int, DtDvbT2Pars&);
...
#define  DATPI_MOD_DVBS2_16APSK		34
#define  DATPI_MOD_DVBS2_32APSK		35
