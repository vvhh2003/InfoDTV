...
TimerProcesser::TimerProcesser(const string aDefineFile,const string aValueFile,const unsigned int aTimerInterval,const SendBufferFunction SendBufferFnPtr,const int aRunMode):
	FDefineFile(aDefineFile),FValueFile(aValueFile),FTimerInterval(aTimerInterval),FTimer(1000,FTimerInterval),
	FTimerCallback(*this, &TimerProcesser::onTimer),FSendBufferFnPtr(SendBufferFnPtr),FRunMode(aRunMode)
	{
...
//	if(!boost::filesystem::exists(DFPath))
	if(!DFPath.exists())
	{
		cout<<"----------->Can't find Define File<---------->"<<FDefineFile<<endl;
		return;
	}
...
//	if(!boost::filesystem::exists(VFPath))
	if(!VFPath.exists())
	{
		cout<<"----------->Can't find Value File<----------"<<FValueFile<<endl;
		return;
	}
	if(FRunMode==0)
